...
Here’s how you can use `sqlsetpgxhelper` to organize your SQL queries.

#### 1. Create SQL Files

First, organize your SQL queries into files. For example, create `queries/users.sql`:
...
Each query is named (e.g., `InsertUser`, `GetByEmail`) and follows the `sqlset` format.

#### 2. Embed SQL Files and Use the Helper

Next, use `//go:embed` to bundle your SQL files with your application and initialize the `sqlsetpgxhelper.DBHelper`.
...
func main() {
	// 1. Create a new SQL set from the embedded filesystem
	queries, err := sqlset.New(queriesFS)
	if err != nil {
		log.Fatalf("Failed to create SQL set: %v", err)
	}

	// 2. Initialize the sqlset-aware DBHelper
	...
	// 4. Execute queries using their path ("setID.queryID")
	_, err = db.Exec(ctx, "users.InsertUser", "Jane Doe", "jane.doe@example.com")
	if err != nil {
	...
	// 5. Get the user back
	var user User
	err = db.Get(ctx, &user, "users.GetByEmail", "jane.doe@example.com")
	if err != nil {
		log.Fatalf("Failed to get user: %v", err)
	}
...
### Using Code Generation for Type-Safe Queries

To avoid using raw strings for `queryPath` and catch errors at compile time, you can use the code generator from the `sqlset` package.

#### 1. Add a `go:generate` Directive

Add a `go:generate` directive to one of your Go files (e.g. `queries.go`). This command will generate a new Go file with constants for each of your SQL queries.

```go
//go:generate go run github.com/istovpets/sqlset/cmd/sqlset-gen --dir=../queries/ --out=queries-gen.go --pkg=consts
```

Run `go generate` in your terminal. This will create `queries-gen.go` with the following content:
...
Now you can replace the string literals in your code with the generated constants for type-safe access.

```go	
	// ...

	// Use generated constants instead of strings
	_, err = db.Exec(ctx, Users.CreateTable)
